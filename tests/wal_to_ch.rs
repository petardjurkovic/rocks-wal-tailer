...
async fn put_in(client: &Client, tbl: &str, k: &str, v: &str) -> clickhouse::error::Result<()> {
    client
        .query(&format!(
            "INSERT INTO srcdb.{tbl} (key, value) VALUES (?, ?)"
        ))
        .bind(k)
        .bind(v)
        .execute()
...
async fn wal_to_clickhouse_discovery_end_to_end_with_op_counts() {
    ...
    client
        .query("CREATE DATABASE IF NOT EXISTS srcdb")
        .execute()
        .await
        .unwrap();
    client
        .query("CREATE DATABASE IF NOT EXISTS wal")
        .execute()
        .await
        .unwrap();

    client
        .query("DROP TABLE IF EXISTS srcdb.kv")
        .execute()
        .await
        .unwrap();
    client
    ...
    client
        .query("DROP TABLE IF EXISTS wal.rdb_changelog")
        .execute()
        .await
        .unwrap();
    client
    ...
    put_in(&client, "kv", "user:1", "A").await.unwrap();
    produced_puts += 1;
    put_in(&client, "kv", "user:1", "B").await.unwrap();
    produced_puts += 1;
    put_in(&client, "kv", "user:2", "X").await.unwrap();
    produced_puts += 1;

    for i in 0..50u32 {
        let k = format!("k{:03}", i);
        put_in(&client, "kv", &k, "v").await.unwrap();
        produced_puts += 1;
    }

    del_in(&client, "kv", "k001").await.unwrap();
    produced_deletes += 1;
    del_in(&client, "kv", "user:2").await.unwrap();
    produced_deletes += 1;

    tokio::time::sleep(Duration::from_millis(5_000)).await;
    put_in(&client, "kv", "user:3", "Z").await.unwrap();
    produced_puts += 1;

    del_in(&client, "kv", "k010").await.unwrap();
    produced_deletes += 1;
    ...
    for _ in 0..80 {
        ...
        if total >= produced_puts + produced_deletes {
            break;
        }
    ...
    assert_eq!(
        captured_puts, produced_puts,
        "PUTs mismatch: produced {}, captured {}",
        produced_puts, captured_puts
    );
    assert_eq!(
        captured_deletes, produced_deletes,
        "DELETEs mismatch: produced {}, captured {}",
        produced_deletes, captured_deletes
    );
    ...
    assert!(
        cnt_user1 >= 2,
        "user:1 should have at least 2 rows, got {}",
        cnt_user1
    );
    ...
    assert_eq!(
        k_distinct, 50,
        "expected 50 distinct k*** keys, got {}",
        k_distinct
    );
...
async fn wal_to_clickhouse_multi_tables_stream_ok() {
    ...
    client
        .query("DROP TABLE IF EXISTS srcdb.kv_a")
        .execute()
        .await
        .unwrap();
    client
        .query("DROP TABLE IF EXISTS srcdb.kv_b")
        .execute()
        .await
        .unwrap();
    ...
    put_in(&client, "kv_a", "user:1", "A").await.unwrap();
    a_puts += 1;
    put_in(&client, "kv_a", "user:1", "B").await.unwrap();
    a_puts += 1;
    put_in(&client, "kv_a", "user:2", "X").await.unwrap();
    a_puts += 1;
    for i in 0..50u32 {
        let k = format!("a{:03}", i);
        put_in(&client, "kv_a", &k, "v").await.unwrap();
        a_puts += 1;
    }
    del_in(&client, "kv_a", "a001").await.unwrap();
    a_dels += 1;
    del_in(&client, "kv_a", "user:2").await.unwrap();
    a_dels += 1;

    let mut b_puts: u64 = 0;
    let mut b_dels: u64 = 0;
    put_in(&client, "kv_b", "user:10", "U").await.unwrap();
    b_puts += 1;
    for i in 0..30u32 {
        let k = format!("b{:03}", i);
        put_in(&client, "kv_b", &k, "w").await.unwrap();
        b_puts += 1;
    }
    del_in(&client, "kv_b", "b005").await.unwrap();
    b_dels += 1;
    put_in(&client, "kv_b", "user:10", "V").await.unwrap();
    b_puts += 1;
    put_in(&client, "kv_b", "user:11", "W").await.unwrap();
    b_puts += 1;

    tokio::time::sleep(Duration::from_millis(1500)).await;
    put_in(&client, "kv_a", "user:3", "Z").await.unwrap();
    a_puts += 1;
    del_in(&client, "kv_a", "a010").await.unwrap();
    a_dels += 1;
    ...
    for _ in 0..80 {
        ...
        for r in &rows {
            if r.src_table == "kv_a" {
                got_a = (r.total, r.puts, r.deletes);
            }
            if r.src_table == "kv_b" {
                got_b = (r.total, r.puts, r.deletes);
            }
        }

        if got_a.0 >= expect_a_total && got_b.0 >= expect_b_total {
            break;
        }
        tokio::time::sleep(Duration::from_millis(500)).await;
    }
...
async fn wal_resume_from_checkpoint_using_helpers() {
    ...
    del_in(&client, "kv", "p003").await.unwrap();
    p1_dels += 1;
    ...
    let ckpt_seq1: u64 = tokio::fs::read_to_string(&ckpt_file)
        .await
        .unwrap()
        .trim()
        .parse()
    ...
    for i in 10..20u32 {
        let k = format!("p{:03}", i);
        put_in(&client, "kv", &k, "v2").await.unwrap();
        p2_puts += 1;
    }
    del_in(&client, "kv", "p011").await.unwrap();
    p2_dels += 1;
    put_in(&client, "kv", "user:2", "X").await.unwrap();
    p2_puts += 1;
    del_in(&client, "kv", "user:2").await.unwrap();
    ...
    let Dist {
        ...
        .query(
            "SELECT count() AS total, countDistinct(seq) AS distinct_seq
                FROM wal.rdb_changelog
                WHERE src_db='srcdb' AND src_table='kv'",
        )
        .fetch_one::<Dist>()
    ...
    assert_eq!(
        total, distinct_seq,
        "seq uniqueness violated (duplicate reprocessing)"
    );
    ...
    let MaxSeq { mx: max_seq2 } = client
        ...
        .await
        .unwrap();
    let ckpt_seq2: u64 = tokio::fs::read_to_string(&ckpt_file)
        .await
        .unwrap()
        .trim()
        .parse()
...
async fn wal_embedded_rocksdb_tuple_pk_end_to_end() {
    ...
    client
        .query("DROP TABLE IF EXISTS srcdb.kv_tuple")
        .execute()
        .await
        .unwrap();
    client
    ...
    // base 5
    put_tuple_in(&client, "kv_tuple", 1, 1, "A1", "init", 10)
        .await
        .unwrap();
    produced_puts += 1;
    touched.insert((1, 1, "A1".into()));
    put_tuple_in(&client, "kv_tuple", 1, 2, "A2", "init", 11)
        .await
        .unwrap();
    produced_puts += 1;
    touched.insert((1, 2, "A2".into()));
    put_tuple_in(&client, "kv_tuple", 2, 1, "B1", "init", 12)
        .await
        .unwrap();
    produced_puts += 1;
    touched.insert((2, 1, "B1".into()));
    put_tuple_in(&client, "kv_tuple", 3, 3, "C1", "init", 13)
        .await
        .unwrap();
    produced_puts += 1;
    touched.insert((3, 3, "C1".into()));
    put_tuple_in(&client, "kv_tuple", 4, 4, "D1", "init", 14)
        .await
        .unwrap();
    produced_puts += 1;
    touched.insert((4, 4, "D1".into()));

    // updates on same PK
    put_tuple_in(&client, "kv_tuple", 1, 1, "A1", "u1", 100)
        .await
        .unwrap();
    produced_puts += 1;
    put_tuple_in(&client, "kv_tuple", 1, 1, "A1", "u2", 101)
        .await
        .unwrap();
    produced_puts += 1;

    // delete one PK
    del_tuple_in(&client, "kv_tuple", 2, 1, "B1").await.unwrap();
    produced_deletes += 1;

    // bulk distinct PKs
    for i in 0..20i64 {
        let k = format!("S{i:02}");
        put_tuple_in(&client, "kv_tuple", 100, i, &k, "v", i * 2)
            .await
            .unwrap();
        produced_puts += 1;
    ...
    put_tuple_in(&client, "kv_tuple", 100, 7, "S07", "v2", 777)
        .await
        .unwrap();
    produced_puts += 1;
    del_tuple_in(&client, "kv_tuple", 100, 5, "S05")
        .await
        .unwrap();
    produced_deletes += 1;
    del_tuple_in(&client, "kv_tuple", 100, 10, "S10")
        .await
        .unwrap();
    produced_deletes += 1;
    ...
    // late insert
    tokio::time::sleep(Duration::from_millis(400)).await;
    put_tuple_in(&client, "kv_tuple", 9, 9, "Z9", "zz", 9)
        .await
        .unwrap();
    produced_puts += 1;
    touched.insert((9, 9, "Z9".into()));
    ...
    let LiveCnt { n: distinct_in_wal } = client
        ...
        .unwrap();
    assert_eq!(
        distinct_in_wal, expected_distinct,
        "distinct key count mismatch"
    );
